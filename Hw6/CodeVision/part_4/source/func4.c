...
void check_acds(){
   ...
   for (i=0;i<8;i++){
        data = read_adc(i);
        //sprintf(str,"Channel%d:%d.%d Mv",i,((ADCH*5)/256) * 1000,(((ADCH*5)/256) * 1000 )%10 ); 
        sprintf(str,"Channel %d:%dMv",i,((data * 5)/1023)*1000 );
        //sprintf(str,"Channel %d:%d Mv",i,data );
        lcd_puts(str);
        delay_ms(1000);
        lcd_clear(); 
        }
...
void func2_interupt(void){
      ...
      for(i = FIRST_ADC_INPUT ;i<= LAST_ADC_INPUT ; i++){
           ...
           if ( ((adc_data[i]* 5)/1023)* 1000 <  tmp_adc[i] * 0.95 ||  ((adc_data[i]* 5)/1023)* 1000 >  tmp_adc[i] * 1.05){
...
{   
    ...
    tmp = ((data*90)/1024) ;
     tmp_value =(2.55 * tmp + 5 ) + 0.5;
    OCR0 = tmp_value; 
    sprintf(str,"Channel %d:%dMv",0,((data * 5)/1023)*1000 );
        //sprintf(str,"Channel %d:%d Mv",i,data );
        lcd_gotoxy(0,0);
        lcd_puts(str);
